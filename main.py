...
if __name__ == '__main__':
    ...
    routers_info = {}
    routers_indexes_list = {}

    # Get local_hostname
    local_hostname_oid = '1.3.6.1.4.1.9.9.23.1.3.4.0'
    local_hostname_request = snmp_operator.snmp_get(snmp_community, R1, local_hostname_oid)
    ...
    raw_local_indexes = snmp_operator.snmp_walk(snmp_community, R1, '1.3.6.1.4.1.9.9.23.1.2.1.1.4')
    ...
    for router_info in routers_info['routers']:
        remote_hostname_oid = '1.3.6.1.4.1.9.9.23.1.2.1.1.6.{}'.format(router_info['index'])
        remote_interface_oid = '1.3.6.1.4.1.9.9.23.1.2.1.1.7.{}'.format(router_info['index'])

        remote_hostname_request = snmp_operator.snmp_walk(snmp_community, R1, remote_hostname_oid)
        for response in remote_hostname_request:
            for oid in response:
                router_info['remote_hostname'] = str(oid[1])

        remote_interface_request = snmp_operator.snmp_walk(snmp_community, R1, remote_interface_oid)
    ...
    build_map(routers_info['routers'])
